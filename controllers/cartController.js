...
exports.cartGet = async (req, res) => {
  try {
    ...
    const coupon = await Coupon.find({
      $and: [{ usedUsers: { $nin: [userId] } }, { showStatus: true }],
    });
    ...
    if (carts) {
      ...
      finalPrice =
        carts.products.reduce((acc, val) => acc + val.totalPrice, 0) + 10;
      couponApplied = await Coupon.findOne({
        couponCode: carts?.couponApplied,
      });
      if (couponApplied !== null) {
        finalPrice = finalPrice - couponApplied.maximumDiscount;
        couponSelected = await Coupon.findOne({
          couponCode: carts?.couponApplied,
        });
      }
    }
...
exports.addToCartPost = async (req, res) => {
  try {
    ...
    if (
      offer &&
      offer?.category?.offer?.expiryDate >= new Date() &&
      !offer?.category?.offer?.isBlocked
    ) {
      let percentage =
        (product.price * offer.category.offer.discountAmount) / 100;
...
exports.updateCartQuantity = async (req, res) => {
  try {
    ...
    if (
      offer &&
      offer?.category?.offer?.expiryDate > new Date() &&
      offer.category.offer.isBlocked == false
    ) {
      let percentage =
        (productData.price * offer.category.offer.discountAmount) / 100;
...
exports.checkout = async (req, res) => {
  try {
    ...
    let finalPrice =
      carts.products.reduce((acc, val) => acc + val.totalPrice, 0) + 10;
    let couponApplied = await Coupon.findOne({
      couponCode: carts?.couponApplied,
    });
    if (couponApplied) {
      finalPrice = finalPrice - couponApplied.maximumDiscount;
